...
for turbine_id in turbine_list:
    ...
    while current_timestamp > start_date:
        ...
        for model in models_group:
            ...
            document[model] = min(model_variable_data.loc[current_timestamp].mean() * 70 , 70)

        document['global_model'] = min(normalized_turbine_data.loc[current_timestamp].mean() * 70, 70)
